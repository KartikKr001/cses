...
ll f(ll x,ll k){
    if(k == 0) return x;
    if(siz[x] > 1){
        return f(v[x],(k-1)%siz[x]);
    }
    return f(v[x],k-1);
}
pp dfs(ll node,ll cnt){
    siz[node] = 1;
    dp[node] = cnt;
    if(prep[node].first != INT_MAX) return prep[node];
    if(dp[v[node]] == -1){
        auto x = dfs(v[node],cnt+1);
        siz[node] = x.second;
        if(node == x.first){
            return prep[node] = {-1,1};
        }
        return prep[node] = x;
    }
    else{
        siz[node] = abs(dp[v[node]]-dp[node])+1;
        return prep[node] = {v[node],siz[node]};
    }
}
int main() {
