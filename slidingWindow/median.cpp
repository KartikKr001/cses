...
multiset<ll>maxa;
void insert(ll ele){
    if(mina.size() == 0 || maxa.size() == 0) mina.insert(ele);
    else if(ele >= *maxa.begin()) maxa.insert(ele);
    else mina.insert(ele);
}
void erase(ll ele){
    auto x = mina.find(ele);
    auto y = maxa.find(ele);
    if(x != mina.end()) mina.erase(x);
    else if(y != maxa.end()) maxa.erase(y);
}
void balance(){
    if(mina.size() > maxa.size()+1){    
        while(mina.size() > maxa.size()+1){
            ll val = *mina.rbegin();
            mina.erase(mina.lower_bound(val));
            maxa.insert(val);
        }
    }
    else if(maxa.size() > mina.size()){
        while(maxa.size() > mina.size()){
            ll val = *maxa.begin();
            mina.insert(val);
            maxa.erase(maxa.lower_bound(val));
        }
    }
}
double getMed(ll n){
    balance();
    if(n%2) return *mina.rbegin();
    else return (min((ll)*mina.rbegin(),(ll)(*maxa.begin())));
}
int main() {
